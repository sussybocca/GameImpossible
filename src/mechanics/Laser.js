...
export class Laser {
    constructor(start, direction, length = 20, color = 0xff0000, damage = 10) {
        ...
        this.damage = damage;
        this.active = true;
        this.reflections = 3; // max reflections

        // Visual: use a line or a series of cylinders for reflected segments
        this.segments = [];
        this.updateGeometry();
    }
    ...
    updateGeometry() {
        ...
        while (remainingLength > 0 && reflectCount <= this.reflections) {
            // Raycast to find hit point with walls
            ...
            const result = window.game.physics.raycast(from, to, { collisionFilterMask: 1 }); // walls only
            let end, hitNormal;
            if (result.hasHit) {
            ...
            if (result.hasHit) {
                ...
                currentStart = end.clone().add(currentDir.clone().multiplyScalar(0.01)); // small offset
                reflectCount++;
            } else {
                break;
            }
    ...
    checkCollision(entity) {
        ...
        // Simple sphere-laser segment collision
        const entityPos = entity.position;
        const radius = entity.radius || 0.5;
        for (let seg of this.segments) {
            // Get line segment from seg's position and orientation
